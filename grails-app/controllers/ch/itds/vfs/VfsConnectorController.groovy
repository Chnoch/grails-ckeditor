...
class VfsConnectorController {
    ...
    def show = {
        def vfsFile = VfsFile.get(params.id)

        response.setHeader("Content-Type", vfsFile.ctype)
        response.setHeader("Content-Length", vfsFile.csize.toString())

        response.outputStream << vfsFile.content.content
    ...
    private quickUpload(baseDir, baseUrl, currentPath, type, request) {
        ...
        if (isUploadEnabled) {
            if (request.method != "POST") {
                ...
                if (!file) {
                    ...
                    if (FileUtils.isAllowed(f.ext, type)) {
                        vfsFile = vfsService.createVfsFileFromMultipartFile(file)
                        vfsFile.save(flush: true)
                    } else {
        ...
        if (!errorMsg) {
            ...
            url = createLink(controller: 'vfsConnector', action: 'show', id: vfsFile.id, absolute: true)
        }

        response.setHeader("Cache-Control", "no-cache")
        render(contentType: "text/html", encoding: "UTF-8") {
            script(type: "text/javascript", "window.parent.CKEDITOR.tools.callFunction(${params.CKEditorFuncNum}, '${url}', '${errorMsg}');")
        }
    }
